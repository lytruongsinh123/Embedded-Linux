...
LINKER_FILE   := $(PRO_DIR)/linker/hung_stm32f407.ld
COMPILER_DIR  := C:/GCC_HUNG

CC  := $(COMPILER_DIR)/bin/arm_none_eabi-gcc
LD  := $(COMPILER_DIR)/bin/arm_none_eabi-ld

INCLUDE_DIRS_OPT  := $(foreach INCLUDE_DIRS, $(INCLUDE_DIRS), I$(INCLUDE_DIRS))

CC_OPT  := -mcpu=cortex-m4 -Wall -c -O0 -g -mtthumb $(INCLUDE_DIRS_OPT)
...
build: $(OJB_FILE) $(LINKER_FILE)
	$(LD) $(LD_OPT) $(PATH_OJBS) -o $(PATH_OUTPUT)/$(PROJ_NAME).elf
	$(COMPILER_DIR)/arm-arm_none_eabi/bin/objcopy.exe -O ihex "$(PATH_OUTPUT)/$(PROJ_NAME).elf" "$(PATH_OUTPUT)/$(PROJ_NAME).hex"
	$(COMPILER_DIR)/arm-arm_none_eabi/bin/objcopy.exe -O binary "$(PATH_OUTPUT)/$(PROJ_NAME).elf" "$(PATH_OUTPUT)/$(PROJ_NAME).bin"
